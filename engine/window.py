...
class Window:
    ...
    _last_windowed_position: Optional[tuple[int, int]] = None
    _last_windowed_size: Optional[tuple[int, int]] = None

    # Viewport
    _viewport: Rect = Rect.empty()
    _viewport_scale: int = 1
    _viewport_texture: Optional[Texture] = None

    # Callbacks
    _resize_callbacks: list[Callable] = []
    _fullscreen_callbacks: list[Callable] = []
    _windowed_callbacks: list[Callable] = []
    ...
    def init(cls, title: str, size: tuple[int, int], flags: int = 0) -> None:
        ...
        # Install callbacks
        if cls.update_viewport not in cls._resize_callbacks:
